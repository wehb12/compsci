...
void main ( void )
{
	...
	gl_FragColor.xyz = ambient + (diffuse_col + spec_col) * float(toBeLit) * spotAdjust;
	gl_FragColor.a = 1.0f;
	...
	//gl_FragColor = vec4(float(toBeLit), float(toBeLit), float(toBeLit), 1);
	//gl_FragColor = vec4(IN.tangent, 1);
}